/* Routines to define and read command-line args and create motif input window.
   ...
   $Id: cmdline.c,v 2.0 1997/03/7 cmk Exp $
   $Log: cmdline.c,v $

 * Revision LL 2.0 March/1997 cmk
 * Fix to fit parameters for the air to ground channel.
 *
	...
	char *progName)
{
	...
	while(cmd != NULL) {
		...
		case cmdFLAG:
	        ...
	        if(y==445)y=y-60;     /* back up since buttons are now row-wise */
                if(y==626)y=y-30;
			break;
		...
		default:	/* cmdINT, cmdFLOAT, cmdCHAR, cmdFILE... */
                  /*printf("y is %d   ",y);*/
		  if(y==505)y=y-59; /* back up to be row-wise.*/
                  /*printf("new y is %d\n",y);*/
