 ...
 * added Lincoln Laboratory to copywrite printout April/1997.
 *
	...
	char *progName)
{
	...
	while(cmd != NULL) {
		...
		case cmdFLAG:
	        ...
	        if(y==505)y=y-70;     /* back up since buttons are now row-wise */
			break;
...
*/
    ...
    if(y==145){
	  ...
	  XtSetArg(args[argCnt],XmNx,220);argCnt++;
	}
	if(y==175){
	  y=145;
	  XtSetArg(args[argCnt],XmNx,295);argCnt++;
	}
    if(y==205){
	  y=145;
	  XtSetArg(args[argCnt],XmNx,370);argCnt++;
	}
    if(y==445){
