/* Routines to define and read command-line args and create motif input window.
   ...
   $Id: cmdline.c,v 2.2 1994/07/25 18:18:32 jjb Exp $
   $Log: cmdline.c,v $
...
int CmdCheckError(char *filename) {
	...
	/* check the radio-boxes */ 
        /* Editted to only warn user if no box selected --CMK 5/23/97 */
	for(i = 0; i < radioBox; i++) 
	  ...
	  if(count != 1) 
          {                     /* if not 1-&-only-1 toggle set... */
            if(count < 1)       /* count = 0 */
	    {
              printf("WARNING FROM CMDLINE:  ");
              printf("NO MEMBER OF RADIO-BOX SET\n");
            }
            else                /* count > 1 */
	    {
	      rtn = -1;	        /* return error condition */
	      printf("Error from Cmdline: ");
	      printf("more than one member of radio-box set\n");
            }
	    printf("    Radio-Box: ");
	    cmd = cmd0;
	    while(cmd != NULL) 
	...
	char *progName)
{
	...
	while(cmd != NULL) {
		...
		case cmdFLAG:
	        ...
	        if(y==295)y=y-70;     /* back up since buttons are now row-wise */
			if(y==605)y=y-65;    /* back up since buttons are now row-wise */
            if(y==660)y=y-89;     /* back up since buttons are now row-wise */
            if(y==661)y=y-25;     /* back up since buttons are now row-wise */ 
			break;
...
*/
    ...
    if(y==295){
	  y=235;
	  XtSetArg(args[argCnt],XmNx,330);argCnt++;
	}
    if(y==545){
	  CmdLabelWidget(shell, bulletinBoard, cmd," Doppler Power Spectrum:", y);
	  XtSetArg(args[argCnt],XmNx,180);argCnt++;
	}
	if(y==575){
	  y=545;
    ...
    if(y==570){
	  /*  CmdLabelWidget(shell, bulletinBoard, cmd," a:", y);*/
	  XtSetArg(args[argCnt],XmNx,100);argCnt++;
	}
	if(y==600){
	  y=570;
	   XtSetArg(args[argCnt],XmNx,175);argCnt++;
	}
    if(y==630){
	  y=570;
    ...
    if(y==660){
	  y=570;
	   XtSetArg(args[argCnt],XmNx,325);argCnt++;
	}
		if(y==631){
	  CmdLabelWidget(shell, bulletinBoard, cmd," Delay Power Spectrum:", y);
	  XtSetArg(args[argCnt],XmNx,180);argCnt++;
	}
	if(y==661){
	  y=631;
	...
	int			y)
{
	...
	XtSetArg(args[argCnt], XmNy, y); argCnt++;
        ...
        XtSetArg(args[argCnt], XmNwidth, 240); argCnt++;
	XtSetArg(args[argCnt], XmNlabelString, (xmStr = XmStringCreateLtoR(
