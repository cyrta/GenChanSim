...
reexport restrictions.
   ...
   $Id: nlosbelrfint.c,v 1.1 1994/09/29 19:17:30 jjb Exp $
   $Log: nlosbelrfint.c,v $
...
static char rcsid[] = "$Id: nlosbelrfint.c,v 1.1 1994/09/29 19:17:30 jjb Exp $";
...
char *argv[];
{
  ...
  fprintf(stdout,"ASSUMPTION: base in center of intersection\n\n");
       ...
       cornerDist;
{
  ...
  double dm, dc;             /* distances over main, cross streets */
  double d;
  double dm2, dc2, dt2;      /* distances used for two turn path */
  double lossTrans;
  ...
  */
  ...
  if ( x < roadWidth/2 ) {
    ...
    /* first try right-left main street to get to mobile */

    dc = y - (roadWidth/2);
    dmBlocks = (int) ((x + (roadWidth/2))/buildSep);
    dm = buildSep * dmBlocks;

    /* check if fall inside a building */
    if ( fabs(x-dm) > (roadWidth/2) ) {

      /* constuct numbers for possible two corner path */

      dm2 = dm;
      tmp = (int) ((y+(roadWidth/2))/buildSep);
      dc2 = tmp * buildSep - (roadWidth/2);
      dt2 = x-dm2-(roadWidth/2);
      dm2 = sqrt( pow(roadWidth/2, 2.0) + pow(dm2, 2.0));
      dc2 = sqrt( pow(roadWidth/2, 2.0) + pow(dc2, 2.0));

      /* single corner in other direction i.e. using up-down main st. */

      dc = x - (roadWidth/2);
      dmBlocks = (int) ((y + (roadWidth/2))/buildSep);
      dm = buildSep * dmBlocks;

      /* check if fall inside a building */
      if ( fabs(y-dm) > (roadWidth/2) ) {
    ...
    /* first try up-down main street to get to mobile */
    ...
    if ( fabs(y-dm) > (roadWidth/2) ) {
      ...
      dm2 = dm;
      tmp = (int) ((x+(roadWidth/2))/buildSep);
      dc2 = tmp * buildSep - (roadWidth/2);
      dt2 = y-dm2-(roadWidth/2);
      dm2 = sqrt( pow(roadWidth/2, 2.0) + pow(dm2, 2.0));
      dc2 = sqrt( pow(roadWidth/2, 2.0) + pow(dc2, 2.0));

      /* single corner in other direction i.e. using right-left main st. */
  ...
  /* adjust end point of dm from center of intersection to center of
     cross street entrance only if cross street used  */

  if ( dc != 0 )
    dm = sqrt( pow(roadWidth/2, 2.0) + pow(dm, 2.0));
  ...
  if ( d > dm )
    lossRG = lossMain + lossCross + 40.0 * log10(d/dm);
  else
    lossRG = lossMain;

  /* compute loss over 2 corners if needed and compare
     select lower */
  ...
  if ( dm2 > 1e-10 ) {
    ...
    lossMain = unobDirUrbHR(frequency, baseHeight, mobileHeight, dm2, slope );

    /* compute loss over cross street */

    if ( dc2 < 1e-10 ) {
      lossCross = 0;
    } else if ( dc2 < cornerDist ) {
      lossCross = cornerLoss * (d - dm2)/cornerDist;
    } else {
      lossCross = cornerLoss;
    }
    ...
    if ( dt2 < 1e-10 ) {
      lossTrans = 0;
    } else if ( dt2 < cornerDist ) {
      lossTrans = cornerLoss * (d - (dm2+dc2))/cornerDist;
    } else {
      lossTrans = cornerLoss;
    }

    lossRG2 = lossMain + lossCross + lossTrans + 40.0 * log10(d/dm);
  ...
  fprintf(stdout,"dm=%lf, dc=%lf\n",dm,dc);
  fprintf(stdout,"dm2=%lf, dc2=%lf, dt2=%lf\n",dm2,dc2,dt2);
  fprintf(stdout,"Lrg2=%lf\n",lossRG2);
  fprintf(stdout,"Lm=%lf, Lc=%lf, Lrg=%lf\n",lossMain,lossCross,lossRG);
#endif
  /* return total loss */
