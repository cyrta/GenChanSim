/*
 * Generate one snapshot of the impulse response of a disturbed HF channel (
 * 
 *

   based on the subroutine with
   copyright (c) 1994 The MITRE Corporation Bedford, MA

   $Id: chfchan.c,v LL0.0 12/1995 cmk Exp $
   $Log: chfchan.c,v $

 * Call the doppler subroutine for both discrete and scatter paths. 7/1/97 CMK
 *
 * Revision LL0.0 12/1995 cmk
 * Changed comments to reflect the mixed model and changed to work with
 * IIR filter coefficients for a Gaussian shaped filter required for the
...
extern void     besselweight(float[],int[],int[],float,float[],float,float,int);

/*static char rcsid[] = "$Id: chfchan.c,v 3.0 1994/08/30 18:37:45 jjb Exp $";*/
static char rcsid[] = "$Id: chfchan.c,v LL0.0 12/1995 cmk Exp $";
...
/*      static char     twfile[100]="classtws.dat";
	...
	if (firstcall) {	/* Do global initialization */
		...
		if ( tw->powSpectrum == GAUSSIAN_SPECTRUM )
		  strcat(warmFile, GAUSSIAN_INTERP_WRM_FILE);
		...
		for (p = 0; p < tw->nPaths; p++) {
			if (tw->mode[p] == NORM) {	/* normal mode */
				...
				besselweight(weight_buff, sttapnum, 
					 tw->nTaps, tw->scatterSampleRate,
					 tw->airHeights, tw->airDist, tw->alpha, p);
			}
		...
		if (firstpath[path] == YES) {	/* if first path weights... */
			...
			for (j = 0; j < NJUMPINGJACKS; j++) {
				cgauss48(&iqbuf[sttapnum[path]], tw->nTaps[path]);
                                iir32(tw, 0, &iqbuf[sttapnum[path]], &tw->filterBank[sttapnum[path]], tw->nTaps[path], sttapnum[path]);
			}
