/* Propagation model for the air to ground channel as described in the
 * 'Generic Channel Simulator Final Report', February 1997
 * under contract MDA904-95-C-2078 by Phillip A. Bello.  The
...
static char rcsid[] = "$Id: airgroundchan.c,v LL0.0 3/1997 $";
...
void main(int argc, char **argv) {
	...
	float   velInplane;  /* aircraft velocity in vertical plane (m/s) */
	float	velPerp;  /* aircraft velocity perpendicular to 
								vertical plane (m/s) */
	/*	float   scatSampRate;*/ /* scatter path sample rate (MHz) */
	float   rmsSlope;    /* RMS slope of surface */
        float   airheightmeters; /* aircraft height in meters */
        float   groundheightmeters; /* ground height in meters */
	...
	CmdFloat("-vhoriz Velocity (in-plane) (m/s)",&velInplane,cmdMan);
	CmdFloat("-vperp Velocity_(perp) (m/s)",&velPerp,cmdMan);
	CmdFloat("-hts Aircraft_Height_(m)",&airheightmeters,cmdMan); 
	CmdFloat("-gndht Ground_Height_(m)",&groundheightmeters,cmdMan); 
	...
	CmdHelp(helpStr = PathFile("/MobileChan/PropChan/AirMobile/airtoground/airgroundchan.hlp"));
	...
	free(helpStr);
        ...
        tw.airHeights[0] = airheightmeters/airDist;
        tw.airHeights[1] = groundheightmeters/airDist;
        printf("Relative aircraft height is: %g.\n",tw.airHeights[0]);
        printf("Relative ground height is: %g.\n",tw.airHeights[1]);
	...
	j = 0;		/* index to packed arrays */
        ...
        if (directPath == 1){        /* There is a direct path */
              DirectDplShift = velInplane * tw.carFreq *
				  ONE_MHZ / (LIGHTSPEED * 
				 sqrt( 1 + (tw.airHeights[0]-tw.airHeights[1])*
			         (tw.airHeights[0]-tw.airHeights[1]) ));     
...
/* Equation (6) in Progress Report 2 */
	...
	for(i = 0; i < othDiscretePaths; i++) {
	  ...
	  /*printf("Discrete paths, j index is %d.\n",j);*/
        ...
        if(scatterPath == 1){
	  distDplShift = velInplane * tw.carFreq * ONE_MHZ 
...
scale it properly according to how the Gaussian IIR filter was designed. */
        termp=velPerp/tw.airHeights[0];
        termp=termp * termp;
        termi=velInplane/tw.airHeights[0];
        termi=termi*termi;
