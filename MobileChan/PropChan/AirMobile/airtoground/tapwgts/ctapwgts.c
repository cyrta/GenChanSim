/* generate tap weights for a single snapshot. 
   ...
   based on a routine ctapwts with
   copyright (c) 1994 The MITRE Corporation Bedford, MA

   $Id: ctapwgts.c,v LL0.0 12/1995 cmk Exp $
   $Log: ctapwgts.c,v $

 * Correct mistake 7/1996 cmk
 * Replaced tw<-scaledsigma to tw<-dplShift for the doppler offset.
 *
 * Revision LL0.0 12/1995 cmk
 * for the air to air channel, the delay power spectrum has a
 * Gaussian shape.
 *
...
static char rcsid[] = "$Id: ctapwgts.c,v LL0.0 12/1995 cmk Exp $";
...
/*		printf("a[%d]= %2.64e\n",i,tw->filterBank[0].sect[i].a );
        ...
        /*printf("CTAPWGTS: before chfchan call\n");*/
	chfchan(tw, wgts, path, interpStep);	/* get tap-weight values */
        /*printf("CTAPWGTS: after chfchan call\n");*/
...
void 
PrintTapWgts(TAPWGTS * tw)
{
	int             i;	/* index */
        printf("\n");
	printf("   Sample Rate (Hz): %8.3f\n", tw->sampleRate);
	if ( tw->model == AIRAIR_MODEL )
	...
	/* both discrete and scatter paths */
	printf("\n");
