...
/*      static char     twfile[100]="classtws.dat";
	...
	if (firstcall) {	/* Do global initialization */
		...
		else if( tw->powSpectrum == GAUSSIAN_SPECTRUM )
		...
		for (p = 1; p < tw->nPaths; p++) {
			sttapnum[p] = sttapnum[p - 1] + tw->nTaps[p - 1];
                ...
                /*printf("CHFCHAN: Before expweight loop.\n");*/
		for (p = 0; p < tw->nPaths; p++) {
		  if (tw->mode[p] == NORM) {	/* discrete mode */
		    /*printf("CHFCHAN: path mode is NORM.\n");*/
		     qphas(tw->nTaps, tw->groupDelay, tw->freqWinTransLen,
			   tw->timeWinTransLen, normresp, sttapnum, p, 
                           tw->sampleRate);
		  }
		  else {                   /* scatter mode */
		    if (tw->model == FLATFADE_MODEL) {
                      /*printf("CHFCHAN: path mode is DIST.\n");*/
	...
	if (tw->mode[path] == DIST) {	/* scatter path mode */
		if (firstpath[path] == YES) {	/* if first path weights... */
		  ...
		  for (j = 0; j < NJUMPINGJACKS; j++) {
		    ...
		    /* printf("CHFCHAN: tw->nTaps[path] is %d.\n",tw->nTaps[path]);
                    printf("CHFCHAN: sttapnum[path] is %d.\n",sttapnum[path]);
                    printf("CHFCHAN: iqbuf after cgauss48 call= %g+i%g.\n",iqbuf[sttapnum[path]].r,iqbuf[sttapnum[path]].i);*/
                                iir32(tw, 0, &iqbuf[sttapnum[path]], &tw->filterBank[sttapnum[path]], tw->nTaps[path], sttapnum[path]);
				/* printf("CHFCHAN: iqbuf after iir32 call= %g+i%g.\n",iqbuf[sttapnum[path]].r,iqbuf[sttapnum[path]].i);*/
			}
