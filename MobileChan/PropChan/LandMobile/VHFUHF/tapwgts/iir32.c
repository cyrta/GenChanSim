/*
   ...
   $Id: iir32.c,v 3.0 1994/08/30 18:30:24 jjb Exp $
   $Log: iir32.c,v $

 * Lincoln Lab Revision  12/1995 Linda Riehl and Cathy Keller
 * added DATA_IIR_WRM_FILE possibility
 *
...
*   as doubles.  They are stored in the file
*   "/MobileChan/PropChan/LandMobile/VHFUHF/tapwgts/dopp_filt.coef".  The
...
/* If first time through */
  ...
  /* whicha is which ratio of vehicle speed has been selected LR Jan 1996 */
		  if (whicha == A25)
			strcat(warmFile, IIR_CLASS25_WRM_FILE);
		  if (whicha == A50)
			strcat(warmFile, IIR_CLASS50_WRM_FILE);
		  if (whicha == A75)
			strcat(warmFile, IIR_CLASS75_WRM_FILE);
		  if (whicha == A100)
			strcat(warmFile, IIR_CLASS100_WRM_FILE);
	}
		if(tw->powSpectrum == DATA_SPECTRUM )
		  strcat(warmFile,DATA_IIR_WRM_FILE);
