...
{	
	...
	if (firstcall) {
		...
		if ((envpath = getenv("SIMDIR")) == (char *) NULL) {
			printf("Error from tapwgts(): must 'setenv SIMDIR ~/hf'\n");
			exit(1);
		}
...
/*		printf("a[%d]= %2.64e\n",i,tw->filterBank[0].sect[i].a );
	...
	chfchan(tw, wgts, path, interpStep,weight_type,data_fname);/* get tap-weight values */
...
/* printf("npaths is %d\n",tw->nPaths); */
	  for(i = 0; i < tw->nPaths; i++) {
	    if(tw->mode[i] == DIST) {
	      printf(" %s\n",data_fname+i*100); /* names are each up to
											   100 chars long*/
	    } else {
	      printf("     N/A ");
	    }
