...
static char rcsid[] = "$Id: mixedchan.c,v LL 0.0 1996 cmk,lbr Exp $";
...
/*	float		multiPathSpr[NPATHS];	*/	/* rms multipath spread (uS) */
	...
	tw.nPathsNorm = 0;           /* init number of discrete paths */
	tw.nPathsDist = 0;           /* init number of scatter paths */
	...
	CmdFlag("-wgen Generate_Warmup_ Filter_Files",&genInterpWarmup,cmdOpt);
	...
	CmdFlag("-classic classic",&jakesSpectrum);
	...
	CmdHelp(helpStr = PathFile("/MobileChan/PropChan/LandMobile/cellular/mixedmod/mixedchan.hlp"));
   ...
   /* set weight_type flag based on Interface flags  Nov 1995 LR */
	...
	if(expPower)weight_type=0;
        ...
        tw.maxdplShift = ((tw.carFreq * ONE_MHZ * tw.VehSpd) / LIGHT_SPEED);
...
/*		if(dplSpread[i] == 0) {
		...
		distDplShift[i] = ((tw.carFreq * ONE_MHZ * tw.VehSpd) / LIGHT_SPEED) 
		  * distDplShift[i];		 /* Fractional Dpl shift for Scatter path */
		tw.dplShift[j] = distDplShift[i];
...
/*		tw.dplSpread[j] = dplSpread[i];			   Dpl spread */  
    ...
    /* last 2 args of call to tapwgts() added by LR Nov 1995 */
 	for(i=0; i<tw.nPaths; ++i)
	  ctapwgts(&tw, wgts, i, 1,weight_type,pow_data_fname,dop_data_fname);
		...
		for(i = 0; i < tw.nPaths; i++) {
		  ...
		  if ( (nSamples != 0) && (interpStep != 0)   ) {
		    ...
		    ctapwgts(&tw, wgts, i, interpStep,weight_type,pow_data_fname,
					 dop_data_fname);			/* get new delay weights */
		    updateCoefFlag = 1;
#ifdef DEBUG
