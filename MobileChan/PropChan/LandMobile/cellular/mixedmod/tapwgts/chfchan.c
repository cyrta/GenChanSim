 ...
 * Revision 1997/07/01 cmk
 * Call the doppler subroutine for both discrete and scatter paths.
 * Therefore, Doppler shifts are applied to scatter path Doppler
 * power spectra.
 *
...
/*      static char     twfile[100]="classtws.dat";
	...
	if (firstcall) {	/* Do global initialization */
		...
		else if( tw->powSpectrum == JAKES_SPECTRUM )
		  strcat(warmFile, JAKES_INTERP_WRM_FILE);
                else
